import Account from './packages/account/model'
import Admin from './packages/admin/model'
import AccountDetail from './packages/account/accountDetailModel'
import Level from './packages/level/model'
import LanguageProgram from './packages/language-program/model'
import DocumentDetail from './packages/document/modelDocumentDetail'
import Document from './packages/document/modelDocument'
import Class from './packages/class/model'
import Course from './packages/course/model'

export {
  Account,
  Admin,
  AccountDetail,
  Level,
  LanguageProgram,
  DocumentDetail,
  Document,
  Class,
  Course
}
